...
import { createWorkArea } from "../../services";
...
export const CreateAreaDialog: React.FC<Props> = ({ isOpen, onClose }) => {
  ...
  const onSubmit = (data: CreateWorkAreaForm) => createWorkAreaMutation(data);

  const onSuccess = () => {
    toast.success("Área de trabajo creada correctamente", {
      description:
        "El área de trabajo se ha creado correctamente, puedes verlo en la lista de áreas de trabajo.",
    });
  ...
  const onError = () => {
    toast.error("Error al crear el área de trabajo", {
      description:
        "Por favor, intenta nuevamente, si el problema persiste, contacta al administrador.",
    });
  };

  const { mutate: createWorkAreaMutation, isPending } = useMutation({
          ...
          <form className="space-y-4 mt-4" onSubmit={handleSubmit(onSubmit)}>
            ...
            <DialogFooter className="mt-4">
              <Button type="submit" disabled={isPending}>
                {isPending ? "Creando..." : "Crear área de trabajo"}
              </Button>
            </DialogFooter>
          </form>
        </DialogHeader>
