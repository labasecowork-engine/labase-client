...
interface Props {
  ...
  initialData?: Partial<EditReminderFormData>;
}
...
export const ReminderForm = forwardRef<HTMLFormElement, Props>(
  ({ onSubmit, initialData }, ref) => {
    ...
    useEffect(() => {
      if (initialData?.send_date) {
        const date = new Date(initialData.send_date!);
    ...
    return (
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Completa los campos para editar el recordatorio.
          </CardDescription>
        </CardHeader>
        <CardContent>
